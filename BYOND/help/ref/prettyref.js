(function(){	// IE shim; this is going away in the future
...
function prevElem(e) {	// superseded in modern browsers by previousElementSibling; ditch this when IE is gone
	e = e.previousSibling;
...
function DMhighlight(pre, params) {
	...
	var dm_keywords_rx = /^(break|new|del|for|global|var|proc|verb|set|static|arg|const|goto|if|in|as|continue|return|do|while|else|sleep|spawn|switch|case|default|tmp|to|try|catch|throw|final)$/;
	params = params || {};
		...
		for(i=sol=sos=0; i<len;) {
			...
			switch(ch) {
				...
				case '*':
					...
					break;
				case '[':
					if(state & 0x32) {push_state(0x10,i-1); continue;}
					break;
				case ']':
...
function makeNav(article) {
	...
	for(i=0; i<name.length;) {
		...
		if(item) {
			...
			a.textContent = item
				.replace(/^.*\((applied to .*)\)$/,'$1')	// turn proc (applied to a matrix) to just "applied to a matrix"
				.replace(/ \(.*\)$/,'')	// get rid of parentheses at end
				.replace(/^(list|matrix)\s+operators$/,'operators')	// change "X operators" to "operators" for certain types
				.replace(/^[^\s\w]+\s+(\w+)\s+operator$/,'$1')	// change "<< input operator" to "input"
				.replace(/\s+(proc|var|statement|operator|filter|parameter|control|setting)s?$/,'')	// change "key var" to "key"
				.replace(/^(proc|var)s$/,"$1")	// change "procs" to "proc"
				.replace(/\s+(?:text\s+)?macro$/,'');
		}
	}
...
function collapse() {
	var body=document.body, refbody;
	if(!window.parent || window.parent==window) {
		body.classList.remove('collapsing');
		if(body.className=='refpage') prettify(body);
		return;
	}
	...
	addCSSRule(ds0, ".refbody", "display: none;", 0);
	addCSSRule(ds0, "body > .article", "width: 100%; height: auto; display: block; position: relative; left: 0; top: 0;", 0);

	var f=document.createDocumentFragment();
	var b,d,i,j,e,n;
	f.appendChild(b=document.getElementById('refbody'));
	f.appendChild(refbody=document.createElement('div'));
	refbody.className='refbody';
	e=b.firstChild;
	for(e=b.firstChild,d=makearticle(refbody); e; e=n) {
		n = e.nextSibling;
		b.removeChild(e);
		if(e.tagName == 'HR') {
			d=makearticle(refbody);
			continue;
		}
	...
	while((e=refbody.querySelector('.article:not([name])'))) refbody.removeChild(e);	// remove articles without names
	while((e=refbody.querySelector('a[name]'))) {	// delete stray named anchors that occur as artifacts
		n = e.parentNode;
		while(i=e.firstChild) n.insertBefore(i,e);
		n.removeChild(e);
	}
	body.appendChild(refbody);
	...
	// unescape is needed for some browsers that might change the ID of an article for lookup purposes from the encoded version to unencoded
	if(document.querySelector('.article[name="/operator/%3e%3e"]')) window.refunescape = function(str) {return str;};
	else window.refunescape = function(str) {return str.replace(/(%[0-9a-f]{2})+/ig, decodeURI);};
	...
	var hashpoll = function() {
		...
		if(h != oldhash) {
		    //console.log('hashpoll',document.location,document.location.hash,h);
			oldhash = h;
			if(activearticle) {
				activearticle.classList.remove('open');
				refbody.appendChild(activearticle);
			}
			activearticle = document.querySelector('.article[name="'+refunescape(h)+'"]');
			...
			if(activearticle) {
				...
				body.appendChild(activearticle);
				if((window.parent||window) != window) window.parent.postMessage("nav:"+h,'*');
				document.documentElement.scrollTop = body.scrollTop = 0;
			}
		}
		...
		try {
			...
			if(sel && !sel.isCollapsed) {
				...
				for(node=sel.anchorNode; node && node!=body; node=node.parentNode) {
					if(node.classList && node.classList.contains('article')) break;
				}
				if(node && node != body && node != activearticle) {
					// save selection
...
function onanimate(fn) {
	if(!window.requestAnimationFrame) return fn;
	return function() {window.requestAnimationFrame(fn);}
}

collapse = onanimate(collapse);
